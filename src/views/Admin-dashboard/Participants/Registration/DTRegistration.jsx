...
const DTRegistration = () => {
    ...
    const updateParticipant = () => {
        UpdateParticipant(participant, firstName, middleName, lastName, nickname, mobile, email, birthday, gender, civilStatus, spouse, religion, baptized, confirmed, memberAddressLine1, memberAddressLine2, memberCity, occupation, specialty, company, companyAddressLine1, companyAddressLine2, companyCity)
            ...
            .then(async result => {
                if (await result.status === 200) {
                    alert(`${firstName} ${middleName !== "" ? middleName.charAt(0) : null}. ${lastName}'s record has been successfully updated!`)
                } else {
                    alert(result.message)
                }
            })
    ...
    const createAttendance = () => {
        CreateAttendanceRecord(loginResult.__, memberId, event, attendance)
            ...
            .then(async result => {
                if (await result.status === 200) {
                    alert(`${firstName} ${middleName !== "" ? middleName.charAt(0) : null}. ${lastName} was successfully tagged as ${attendance}`)
                } else {
                    alert(result.message);
                }
            })
    ...
    const resetAttendance = (attendance, firstName, middleName, lastName) => {
        DeleteAttendanceRecord(attendance)
            ...
            .then(async result => {
                if (await result.status === 200) {
                    alert(`${firstName} ${middleName !== "" ? middleName.charAt(0) : null}. ${lastName}'s attendance record has been reset.`);
                } else {
                    alert(result.message);
                }
            })
                            ...
                            attendanceRecord.map(items => (
                                <tr>
                                    <td className="px-7 py-3">
                                        {items.first_name} {items.middle_name !== null ? items.middle_name.charAt(0) : null}. {items.last_name}
                                    </td>
                                    <td className="px-7 py-3">
                                        {items.birthday}
                                    </td>
                                            ...
                                            :
                                                attendanceRecord.map(getSpouse => (
                                                    getSpouse.member_id === items.spouse_member_id ?
                                                        getSpouse.first_name + ' ' + (getSpouse.middle_name !== null ? getSpouse.middle_name.charAt(0) : null) + ' ' + getSpouse.last_name
                                                    : null
                                                ))
            ...
            <AttendanceWarning show={showAttendanceWarning} setShow={closeAttendanceWarning}>
                <div className="flex items-center justify-around gap-4 mb-4 sm:grid-cols-2 rounded-lg">
                        <div className='flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg'>
                            ...
                            <label className='font-semibold text-lg'>Are you sure you want to tag <strong><em>"{firstName} {middleName !== undefined & middleName !== "" & middleName !== "" ? middleName.charAt(0) : null}. {lastName}"</em></strong> as <strong><em>"{attendance}"</em></strong>?</label>
                        </div>
                    </div>
                    <div className="flex items-center justify-around gap-4 mb-4 sm:grid-cols-2 rounded-lg">
                        <div className='flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg'>
