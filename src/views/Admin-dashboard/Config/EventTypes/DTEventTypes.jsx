...
const DTEventTypes = () => {
        ...
        AddNewEventTypes(loginResult.__, eventTypeName, eventTypeCategory)
            ...
            .then(async result => {
                if (await result.status === 200) {
                    ...
                    alert(`${eventTypeName} already exist.`)
                } else {
    ...
    const editEventType = (eventTypeId, eventTypeName, eventTypeCategory) => {
    ...
    const updateEventType = () => {
        ...
        UpdateEventType(eventTypeId, eventTypeName, eventTypeCategory)
            ...
            .then(async result => {
                if (await result.status === 200) {
                    alert('Record was successfull updated!');
                } else if(await result.status === 422) {
                    alert(`No changes were made.`)
                } else {
                    alert('An error occured while trying to update the records. Please contact system administrators.')
                }
            });
        ...
        <div className="mt-10 relative overflow-x-auto shadow-md sm:rounded-lg">
            <div className="flex items-center justify-between py-4 px-2 bg-white dark:bg-gray-900">
                ...
                <div>
                    ...
                    <span className="flex-1 ml-3 whitespace-nowrap">Add New Event Type</span>
                    </button>
                </div>
            </div>
                                    ...
                                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className='hover:cursor-pointer'>
                                            {
                                                eventType.map(category => (
                                                    category.event_type_id === items.event_type_category ?
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap" style={{ cursor: "pointer", width: "20%" }}>
                                            ...
                                            <button type="button"
                                                ...
                                                onClick={() => editEventType(items.event_type_id, items.event_type_name, items.event_type_category)}
                                            >
                                                <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <AddEventTypes show={showAdd} setShow={closeAdd}>
                <div className="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        <div className="relative">
                            <select id="pi_spouse" className=" md bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                onChange={(e) => {setEventTypeCategory(e.target.value)}}
                                value={
                            ...
                            >
                                <option selected value={" "}>Choose Event Type Category</option>
                                {
                                    status ?
                                        eventType.map(items => (
                                                items.event_type_category === null ?
